...
def extract_experience(text):
    ...
    for line in lines:
        # Check for exact whole words using regex word boundaries
...
def extract_skills(text):
    ...
    for line in lines:
        ...
        if any(keyword in lower_line for keyword in ["skills", "technical skills", "key skills","proficient with"]):
            capturing = True
...
def extract_certifications(text):
    ...
    return ' | '.join(certs[:3])  # Return top 3 certifications
...
def upload():
    ...
    with open(CSV_FILE, 'w', newline='', encoding='utf-8') as csvfile:
        # Match the order of your HTML table headers
        fieldnames = [
            "Certifications",
            "Education", 
            "Email",
        ...
        # Reorder the data to match CSV headers
